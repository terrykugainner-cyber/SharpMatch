...
def test_basic_ncc():
    ...
    # 基本 NCC - 計算完整 scoremap
    score = ncc_scoremap(tpl, img)
    
    # Pyramid search for single best match (demonstration)
    x_best, y_best, sc_best = pyramid_ncc_search(tpl, img)
    print(f'[最佳匹配 from pyramid] x={x_best}, y={y_best}, score={sc_best:.3f}')
    
    # Extract multiple peaks from scoremap (multi-match)
    ...
    for i, (px, py, sc_peak) in enumerate(peaks):
        if sc_peak < 0.85:
            continue
        match_count += 1
        vis = draw_box(vis, px, py, W, H, text=f'#{match_count} {sc_peak:.3f}')
        print(f'[MATCH {match_count}] x={px}, y={py}, score={sc_peak:.3f}')
    
    cv2.imwrite('examples/test_ncc_basic.png', vis)
    print(f'[DONE] Found {match_count} matches (score >= 0.85), saved to examples/test_ncc_basic.png')
...
def test_roi_search():
    ...
    # ROI NCC - 計算完整 scoremap
    score_roi = ncc_scoremap(tpl, img, roi=roi)
    x_best, y_best, sc_best = pyramid_ncc_search(tpl, img, roi=roi)
    print(f'[最佳匹配 from pyramid] x={x_best}, y={y_best}, score={sc_best:.3f}')
    
    # Extract multiple peaks from ROI scoremap
    K = 150
    min_dist = max(tpl_h, tpl_w) // 2
    peaks = topk_peaks(score_roi, K=K, min_dist=min_dist)
    ...
    for i, (px, py, sc_peak) in enumerate(peaks):
        if sc_peak < 0.85:
            continue
        match_count += 1
        # ROI scoremap 的座標需要加上 ROI 偏移
        x_match = px + x1
        y_match = py + y1
        vis = draw_box(vis, x_match, y_match, tpl_w, tpl_h, text=f'#{match_count} {sc_peak:.3f}')
        print(f'[MATCH {match_count}] x={x_match}, y={y_match}, score={sc_peak:.3f}')
    
    cv2.imwrite('examples/test_ncc_roi.png', vis)
    print(f'[DONE] Found {match_count} matches (score >= 0.85), saved to examples/test_ncc_roi.png')
...
def test_mask_support():
    ...
    for mask_name, mask in masks.items():
        ...
        # 使用 mask 的 NCC - 計算完整 scoremap
        score_mask = ncc_scoremap(tpl, img, mask=mask)
        x_best, y_best, sc_best = pyramid_ncc_search(tpl, img, mask=mask)
        
        print(f"  [最佳匹配 from pyramid] x={x_best}, y={y_best}, score={sc_best:.3f}")
        
        # Extract multiple peaks from mask scoremap
        K = 150
        min_dist = max(tpl_h, tpl_w) // 2
        peaks = topk_peaks(score_mask, K=K, min_dist=min_dist)
        
        results[mask_name] = peaks
        ...
        for i, (px, py, sc_peak) in enumerate(peaks):
            if sc_peak < 0.85:
                continue
            match_count += 1
            vis = draw_box(vis, px, py, tpl_w, tpl_h, text=f'#{match_count} {sc_peak:.3f}')
        
        axes[2].imshow(cv2.cvtColor(vis, cv2.COLOR_BGR2RGB))
        axes[2].set_title(f'Match Results ({match_count} matches, score >= 0.85)')
        axes[2].axis('off')
        
        print(f"  找到 {match_count} 個匹配 (score >= 0.85)")
...
def test_roi_and_mask():
    ...
    # 使用 ROI + Mask 的 NCC - 計算完整 scoremap
    score_combo = ncc_scoremap(tpl, img, roi=roi, mask=mask)
    x_best, y_best, sc_best = pyramid_ncc_search(tpl, img, roi=roi, mask=mask)
    
    print(f"[最佳匹配 from pyramid] x={x_best}, y={y_best}, score={sc_best:.3f}")
    
    # Extract multiple peaks from ROI+Mask scoremap
    K = 150
    min_dist = max(tpl_h, tpl_w) // 2
    peaks = topk_peaks(score_combo, K=K, min_dist=min_dist)
    ...
    axes[1].set_title(f'Match Results (ROI+Mask, {match_count} matches)')
    axes[1].axis('off')
    ...
    plt.close()
    print(f"[DONE] Found {match_count} matches (score >= 0.85), saved to examples/test_ncc_roi_mask_combo.png")
